class HistoryManager {
    ...
    applyAction(action, isUndo) {
        ...
        } else if (action.type === 'moveBackground' || action.type === 'resizeBackground' || action.type === 'rotateBackground') {
            if (window.background) {
...
// Helper function to add touch-friendly event handler (works on both touch and mouse)
function addTouchClickHandler(element, handler) {
    ...
    // Touch events for mobile/iPad - PRIMARY HANDLER
    ...
    element.addEventListener('touchend', (e) => {
        ...
        // Only trigger if it was a quick tap (not a swipe) and didn't move much
...
class App {
    ...
    init() {
        ...
        // Set app instance on window for global access
        window.app = this;
        ...
        console.log('Pixel Art Editor initialized');
    }
    ...
    setupUndoRedo() {
        ...
        addTouchClickHandler(redoBtn, () => {
            if (window.historyManager && window.historyManager.historyIndex < window.historyManager.history.length - 1) {
    ...
    showZoomInputModal(onConfirm) {
        ...
        if (!modal || !input) {
            console.error('Zoom input modal not found');
            return;
        }

        // Set current zoom value
        const currentValue = Math.round(this.zoomLevel * 100);
        ...
        const closeModal = (result) => {
            ...
            if (result && onConfirm) {
                ...
                if (!isNaN(value) && value >= 10 && value <= 400) {
                    ...
                    alert('Please enter a value between 10 and 400.');
                }
            }
        };
    ...
    setupTraceButton() {
        ...
        const showConfirmation = () => {
            if (!window.background || !window.background.image) {
                console.log('No background image loaded. Please import an image first.');
    ...
    setupCanvasInfoClick() {
        ...
        const handleClick = (e) => {
            ...
            console.log('Canvas info clicked');
            ...
            // Show modal
            if (window.canvasSizeModal) {
                console.log('Showing canvas size modal');
...
if (document.readyState === 'loading') {
    ...
    // Align pan button after initial load (in case grid isn't initialized yet)
    setTimeout(() => {
        if (window.app) {
            window.app.alignPanButtonWithSaveLoadButtons();
        }
    }, 500);
}
