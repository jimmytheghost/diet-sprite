class Grid {
    ...
    getCellFromEvent(e) {
        ...
        // Debug logging when zoomed and panned
        if (this.zoom > 1 && (panX !== 0 || panY !== 0)) {
            console.log('Cell calculation:', {
                clientX: e.clientX,
                clientY: e.clientY,
                rectLeft: rect.left,
                rectTop: rect.top,
    ...
    setPixelData(data) {
        ...
        if (data.length !== this.height || (data[0] && data[0].length !== this.width)) {
            console.warn(`setPixelData: Data size (${data.length}x${data[0] ? data[0].length : 0}) doesn't match grid size (${this.height}x${this.width}). Resizing...`);
            // Resize data to match grid size
            const resizedData = Array(this.height).fill(null).map(() => Array(this.width).fill(null));
    ...
    shiftPixels(deltaX, deltaY) {
        ...
        const newPixels = Array(this.height).fill(null).map(() => Array(this.width).fill(null));
    ...
    updateTraceProgress(processed, total, drawn, skipped) {
        ...
        if (progressText) progressText.textContent = `Processed ${processed.toLocaleString()} of ${total.toLocaleString()} pixels`;
        if (statusText) statusText.textContent = `${drawn} pixels drawn, ${skipped} white pixels skipped`;
    }
    ...
    stopTrace() {
        if (this.traceCancelled !== undefined) {
            ...
            console.log('Trace operation cancelled by user');
        }
    }
    ...
    traceBackgroundImage() {
        if (!window.background || !window.background.image) {
            console.log('No background image to trace');
            return;
        }
        ...
        const processBatch = () => {
            ...
            if (this.traceCancelled) {
                console.log('Trace cancelled - stopping');
                return;
            }
            ...
            if (currentIndex < pixelsToProcess.length && !this.traceCancelled) {
                ...
                if (!this.traceCancelled) {
                    ...
                    this.hideTraceProgress();
                    console.log(`Trace cancelled: ${changes.length} pixels drawn before cancellation`);
                }
            }
        };
    ...
    finishTrace(changes, skippedWhitePixels) {
        ...
        setTimeout(() => {
            this.hideTraceProgress();
            console.log(`Trace complete: ${changes.length} pixels drawn, ${skippedWhitePixels} white pixels skipped`);
        }, 500);
    }
}
