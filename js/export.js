class Export {
  ...
  init() {
    document
      ...
      .addEventListener('click', () => this.showExportModal('png'));
  ...
  showExportModal(type) {
    ...
    if (this.exportBgModal) {
      this.exportBgModal.open();
    } else {
  ...
  createExportOptionButton(option, type) {
    ...
    btn.onclick = () => {
      ...
      if (this.exportBgModal) {
        this.exportBgModal.close(false);
      } else {
  ...
  exportPNG(_bgOption = 'transparent', bgColor = null) {
  ...
  showSVGExportModal() {
    ...
    if (this.svgExportModal) {
      this.svgExportModal.open();
    } else {
      modal.style.display = 'flex';
    }
  }
  ...
  createSVGExportOptionButton(sizeType, label, dimensions, description) {
    ...
    btn.onclick = () => {
      ...
      if (this.svgExportModal) {
        this.svgExportModal.close(false);
      } else {
  ...
  setupExportAllModal() {
    ...
    const closeModal = () => {
      if (this.exportAllModal) {
        this.exportAllModal.close(false);
      } else {
    ...
    if ('showDirectoryPicker' in window) {
      try {
        ...
        pngCanvas.toBlob(async (blob) => {
          const fileHandle = await directoryHandle.getFileHandle(`${sanitizedFileName}.png`, {
            create: true
          });
        ...
        const svgFileHandle = await directoryHandle.getFileHandle(`${sanitizedFileName}.svg`, {
          create: true
        });
        ...
        const jsonFileHandle = await directoryHandle.getFileHandle(`${sanitizedFileName}.json`, {
          create: true
        });
        ...
        console.info(`Exported all files to: ${sanitizedFileName}.png, .svg, .json`);
      } catch (err) {
