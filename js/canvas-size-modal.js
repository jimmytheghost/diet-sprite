...
class CanvasSizeModal {
  ...
  getValidationMessageEl() {
    let messageEl = document.getElementById('canvasSizeValidationMessage');
    if (!messageEl && this.modal) {
      const confirmBody = this.modal.querySelector('.confirm-body');
      if (confirmBody) {
        messageEl = document.createElement('p');
        messageEl.id = 'canvasSizeValidationMessage';
        messageEl.style.margin = '0 0 12px 0';
        messageEl.style.fontSize = '11px';
        messageEl.style.color = '#ff6b6b';
        messageEl.style.textAlign = 'left';
        messageEl.style.display = 'none';
        confirmBody.prepend(messageEl);
      }
    }
    return messageEl;
  }
  ...
  showValidationMessage(message) {
    const messageEl = this.getValidationMessageEl();
    if (!messageEl) return;
  ...
  clearValidationMessage() {
    const messageEl = document.getElementById('canvasSizeValidationMessage');
    if (!messageEl) return;
  ...
  confirm() {
    ...
    if (widthInput && heightInput) {
      ...
      if (isNaN(width) || isNaN(height)) {
        this.showValidationMessage('Please enter valid numbers for width and height.');
        widthInput.focus();
        return;
      }
      ...
      if (width < 8 || width > 1024) {
        this.showValidationMessage('Width must be between 8 and 1024 pixels.');
        widthInput.focus();
        widthInput.select();
        return;
      }
      ...
      if (height < 8 || height > 1024) {
        this.showValidationMessage('Height must be between 8 and 1024 pixels.');
        heightInput.focus();
        heightInput.select();
        return;
      }
