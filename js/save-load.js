class SaveLoad {
    ...
    setupSaveProjectModal() {
        ...
        const closeBtn = document.getElementById('saveProjectClose');
        const cancelBtn = document.getElementById('saveProjectCancel');
    ...
    newProject() {
        ...
        if (window.location.pathname.includes('/custom/')) {
            ...
            if (window.palette) {
                ...
                document.querySelectorAll('.palette-color.selected, .custom-color-slot.selected').forEach(el => {
            ...
            console.log('New project - showing canvas size modal');
            return;
        }
        ...
        reader.onload = (e) => {
            try {
                ...
                if (!savedWidth || !savedHeight) {
                    if (projectData.grid.pixels && Array.isArray(projectData.grid.pixels) && projectData.grid.pixels.length > 0) {
                ...
                if (window.location.pathname.includes('/custom/') && window.app && window.app.initializeGrid) {
                ...
                if (window.background && projectData.background) {
                    if (projectData.background.imageData) {
                        ...
                        img.onload = () => {
                            ...
                            window.background.x = projectData.background.x !== undefined ? projectData.background.x : 0;
                            window.background.y = projectData.background.y !== undefined ? projectData.background.y : 0;
                            window.background.scale = projectData.background.scale !== undefined ? projectData.background.scale : 1.0;
                            window.background.rotation = projectData.background.rotation !== undefined ? projectData.background.rotation : 0;
                ...
                if (window.grid && projectData.settings) {
                    ...
                    const fadeOpacity = projectData.settings.fadeToBlack !== undefined ? projectData.settings.fadeToBlack : 100;
    ...
    async showSVGLoadConfirmModal(message) {
    ...
    async loadSVG(svgContent) {
        try {
            ...
            // Get SVG dimensions
            const svgWidth = parseFloat(svgElement.getAttribute('width')) || parseFloat(svgElement.getAttribute('viewBox')?.split(' ')[2]) || 640;
            const svgHeight = parseFloat(svgElement.getAttribute('height')) || parseFloat(svgElement.getAttribute('viewBox')?.split(' ')[3]) || 640;
            ...
            // Validate grid dimensions
            if (finalGridWidth < 8 || finalGridWidth > 1024 || finalGridHeight < 8 || finalGridHeight > 1024) {
                throw new Error(`Invalid grid dimensions: ${finalGridWidth}x${finalGridHeight}. Must be between 8-1024 pixels.`);
            }
            
            // Initialize pixel array
            const pixels = Array(finalGridHeight).fill(null).map(() => Array(finalGridWidth).fill(null));
            ...
            allRects.forEach(rect => {
                ...
                const isPixelAligned = 
                    width > 0 && width < 100 && // Reasonable pixel size
                    height > 0 && height < 100 &&
            ...
            if (window.grid && (window.grid.width !== finalGridWidth || window.grid.height !== finalGridHeight)) {
            ...
            if (window.background && backgroundElements.length > 0) {
                const backgroundSVG = this.createBackgroundSVG(svgElement, backgroundElements, svgWidth, svgHeight);
    ...
    parsePathToRect(path) {
        ...
        const match = pathData.match(/m\s*([-\d.]+)\s+([-\d.]+)\s*h\s*([-\d.]+)\s*v\s*([-\d.]+)\s*h\s*-([-\d.]+)\s*z/i);
        if (match) {
            ...
            if (Math.abs(width - parseFloat(match[5])) < 0.1 && 
                width > 0 && height > 0 && 
        ...
        const match2 = pathData.match(/m\s*([-\d.]+)\s+([-\d.]+)\s*v\s*([-\d.]+)\s*h\s*([-\d.]+)\s*v\s*-([-\d.]+)\s*h\s*-([-\d.]+)\s*z/i);
        if (match2) {
            ...
            if (Math.abs(height - parseFloat(match2[5])) < 0.1 &&
